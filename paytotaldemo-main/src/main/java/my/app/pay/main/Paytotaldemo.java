...
public class Paytotaldemo {
    ...
    public static String getAppBaseDir() {
        ...
        try {
            ...
            if (dir == null) {
                String name = Paytotaldemo.class.getName().replace('.', '/') + ".class";
                URL url = Paytotaldemo.class.getClassLoader().getResource(name);
                String file = new File(url.toURI()).getAbsolutePath();
                int pos = file.indexOf("paytotaldemo-main");
